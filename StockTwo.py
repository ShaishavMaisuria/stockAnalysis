...
company='yahoo'
#stock="GOOGL"
stock=['AAPL','GE','GOOG','IBM','MSFT']
dataframe=web.DataReader(stock,str(company),start=start,end=end)['Adj Close']
...
plt.figure(200)
returns = close_d / close_d.shift(1) - 1

returns.plot(label='return')
plt.show()

with open('stock_file.txt', mode='a') as stock_file:
    stock_file.write(str(movingAverage))

rcomp=dataframe.pct_change()
correlation=rcomp.corr()
print(correlation)

plt.figure(203)
plt.scatter(rcomp.AAPL,rcomp.GE)
